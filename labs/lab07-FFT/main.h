...
TYPE* genRandomSignal(int N){
    TYPE* array = (TYPE*)malloc(sizeof(TYPE)*N);
    for(int i=0; i<N; ++i){
        array[i] = (((TYPE)rand()/RAND_MAX) - 0.5)*2.0;
    }
    return array;
}

void printArray(TYPE* a, int N, const char *msg){
    if(N <= PRINT_LIMIT){
        printf("%s:\n");
        for(int i=0; i<N; ++i){
            a[i] = (((TYPE)rand()/RAND_MAX) - 0.5)*2.0;
        }
    }
}
